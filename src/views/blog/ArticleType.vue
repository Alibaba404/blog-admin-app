<template>
  <section>
    ...
    >
      ...
      <el-table-column label="状态" width="150">
        <template scope="scope">
          <el-tag type="success" size="medium" v-if="scope.row.status == 1">启用</el-tag>
          <el-tag type="danger" size="medium" v-else="scope.row.status == 0">未启用</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="createTime" label="创建时间" width="200"></el-table-column>
      <el-table-column prop="updateTime" label="更新时间" width="300" show-overflow-tooltip></el-table-column>
      <!-- <el-table-column prop="parentId" label="父类型ID" width="300" show-overflow-tooltip></el-table-column> -->
      <el-table-column prop="parentName" label="父类型名称" width="300" show-overflow-tooltip></el-table-column>
    ...
    <!--添加或修改界面-->
    <el-dialog title="保存文章类型" :visible.sync="saveFormVisible" :close-on-click-modal="false">
      <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="saveForm">
        <el-form-item label="类型名称" prop="typeName">
          <el-input v-model="saveForm.typeName" auto-complete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="父类型" prop="phone">
...
export default {
  ...
  methods: {
    ...
    handleDel(row) {
      if (row.id) {
        this.$confirm(`确认删除${row.typeName}信息吗?`, "提示", {
          type: "warning",
        })
          .then((resp) => {
            this.$http
              .delete(`/articleType/${row.id}`)
              .then((resp) => {
                if (resp.data.success) {
      ...
      this.$http
        ...
        .then(({ data }) => {
          if (data.success) {
            ...
            this.doCmdDisabled(this.allTypes, id);
          }
        })
        .catch((resp) => {});
    },
    /**
     *  禁用当前编辑的类型(不能选择自己和子孙类型)
     * @param {array} types 待查询的数组
     * @param {number} id 类型ID
     */
    doCmdDisabled(types, id) {
      for (let i = 0; i < types.length; i++) {
        const e = types[i];
        if (e.children) {
          this.doCmdDisabled(e.children, id);
        }
        if (id == e.id) {
          // 禁用自己
          e.disabled = true;
          // 清空当前类型下面的子类型(不能选择自己同时也不能选择子类)
          e.children = null;
        }
      }
    },
