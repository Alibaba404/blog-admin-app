<template>
  <section>
    ...
    >
      ...
      <el-table-column prop="component" label="Vue组件路径" width="320"></el-table-column>
      <el-table-column prop="url" label="菜单路由地址" width="220"></el-table-column>
      <el-table-column label="菜单图标" width="120">
        <template scope="scope">
          <i :class="scope.row.icon"></i>
        </template>
      </el-table-column>
      <el-table-column prop="index" label="菜单排序" width="120"></el-table-column>
      <el-table-column prop="parentName" label="上级菜单" width="120"></el-table-column>
    ...
    <!--添加或修改界面-->
    <el-dialog title="保存菜单" :visible.sync="saveFormVisible" :close-on-click-modal="false">
      <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="saveForm">
        ...
        <el-form-item label="菜单排序">
          <el-input-number v-model.number="saveForm.index" :min="0" :max="99" label="菜单排序..."></el-input-number>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleDel(row) {
      if (row.id) {
        this.$confirm(`确认删除菜单${row.username}吗?`, "提示", {
          type: "warning",
        })
          .then((resp) => {
            this.$http
      ...
      this.$http
        ...
        .then(({ data }) => {
          if (data.success) {
            ...
            this.doCmdDisabled(this.allMenu, id);
          }
        })
        .catch((resp) => {});
    },
     ...
     * @param {array} eles 待查询的数组
     * @param {number} id 类型ID
     */
    doCmdDisabled(eles, id) {
