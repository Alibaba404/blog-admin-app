<template>
  <section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px">
      <el-form :inline="true">
        <el-form-item>
          <el-input v-model="keyword" placeholder="请输入权限名称..."></el-input>
        </el-form-item>
        <el-form-item>
    ...
    >
      ...
      <el-table-column prop="name" label="权限名称" width="120"></el-table-column>
      <el-table-column prop="url" label="权限接口地址"></el-table-column>
      <el-table-column prop="sn" label="权限编码"></el-table-column>
      <el-table-column prop="descs" label="权限描述"></el-table-column>
    ...
    <!--添加或修改界面:权限无新增,由后台自动生成-->
    <el-dialog title="保存权限" :visible.sync="saveFormVisible" :close-on-click-modal="false">
...
export default {
  data() {
    return {
      ...
      //6.添加和修改校验规则-表单项校验
      saveFormRules: {
        username: [{ required: true, message: "请输入权限账号", trigger: "blur" }],
        phone: [{ required: true, message: "请输入权限电话号码", trigger: "blur" }],
      },
    };
  },
  methods: {
    ...
    handleDel(row) {
      if (row.id) {
        this.$confirm(`确认删除权限${row.username}吗?`, "提示", {
          type: "warning",
        })
          .then((resp) => {
            this.$http
              .delete(`/permission/${row.id}`)
              .then((resp) => {
                if (resp.data.success) {
